...
class VectorIndexService:
    ...
    def index_directory(self, directory_path: Optional[str] = None) -> IndexingResult:
        ...
        try:
            ...
            # 获取所有支持的文件（统一由 DocumentLoader 判断）
            all_files = list(dir_path.iterdir())
            files = [f for f in all_files if f.is_file() and document_loader.is_supported(str(f))]

            if not files:
                supported_exts = document_loader.TEXT_EXTS | document_loader.DOCX_EXTS | document_loader.PDF_EXTS | document_loader.IMAGE_EXTS
                logger.warning(f"目录中没有找到支持的文件: {target_path}，支持格式: {supported_exts}")
    ...
    def index_single_file(self, file_path: str):
        ...
        try:
            # 1. 统一文档加载器（支持 txt/md/docx/pdf/图片）
            content = document_loader.load(str(path))
            logger.info(f"文档内容提取完成: {path}, 内容长度={len(content)} 字符")
